...
*discpy* logs errors and debug information via the :mod:`logging` python
...
``discpy`` as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``discpy.log`` instead of
outputting them to the console the following snippet can be used::

    import discpy
    import logging

    logger = logging.getLogger('discpy')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='discpy.log', encoding='utf-8', mode='w')
