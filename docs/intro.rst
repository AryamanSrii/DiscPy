...
This is the documentation for ``discpy``, a library for Python to aid
in creating applications that utilise the Discord API.

Prerequisites
---------------

``discpy`` works with Python 3.8 or higher. Support for earlier versions of Python
...
To get voice support, you should use ``discpy[voice]`` instead of ``discpy``, e.g. ::
...
discpy revolves around the concept of :ref:`events <discpy-api-events>`.
