...
Working with :ref:`discpy_api_models` is a fairly common thing when defining commands, as a result the library makes
...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: discpy.Member):
...
A lot of discpy models work out of the gate as a parameter:
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[discpy.TextChannel, discpy.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`discpy.TextChannel` converter or a :class:`discpy.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`discpy.TextChannel`, and if it fails it tries to convert it to a :class:`discpy.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[discpy.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[discpy.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`discpy.Member` followed by a
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[discpy.Member] = commands.flag(name='member', default=lambda ctx: [])
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[discpy.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
:func:`.on_command_error` which works like any other event in the :ref:`discpy-api-events`. This global error handler is
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: discpy.Member):
