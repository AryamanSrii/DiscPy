...
def create_overwrites(ctx, *objects):
    ...
    A `discpy.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `discpy.Role` or a `discpy.Member`.
    ...
    # for each `discpy.Role` or `discpy.Member`.
    overwrites = {obj: discpy.PermissionOverwrite(view_channel=True) for obj in objects}
    ...
    overwrites.setdefault(
        ctx.guild.default_role, discpy.PermissionOverwrite(view_channel=False)
    )

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = discpy.PermissionOverwrite(view_channel=True)
...
@commands.guild_only()
async def text(
    ctx: commands.Context, name: str, *objects: typing.Union[discpy.Role, discpy.Member]
):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.Context, emoji: discpy.PartialEmoji, *roles: discpy.Role):
