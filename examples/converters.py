...
@bot.command()
async def userinfo(ctx: commands.Context, user: discpy.User):
    # In the command signature above, you can see that the `user`
    # parameter is typehinted to `discpy.User`. This means that
    # during command invocation we will attempt to convert
    # the value passed as `user` to a `discpy.User` instance.
    # The documentation notes what can be converted, in the case of `discpy.User`
    ...
    # If the conversion is successful, we will have a `discpy.User` instance
...
class ChannelOrMemberConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str):
        ...
        # `discpy.Member` or `discpy.TextChannel` instance from the
...
@bot.command()
async def ignore(
    ctx: commands.Context, target: typing.Union[discpy.Member, discpy.TextChannel]
):
    ...
    # So, it will attempt to convert whatever is passed to `target` to a `discpy.Member` instance.
    # If that fails, it will attempt to convert it to a `discpy.TextChannel` instance.
